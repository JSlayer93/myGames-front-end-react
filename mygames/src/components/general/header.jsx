...
const Header = () => {
    ...
    return(
        <section className="bg-zinc-500 h-[7vh] w-screen fixed z-50 opacity-80 flex flex-row items-center">
            <nav className="flex-initial w-7/12">
            ...
            <nav className="w-7/12 flex justify-around text-white text-[2vh]">
                ...
                <form onSubmit={(e) => {navigate(e.target[0].value ? `/search/${e.target[0].value}` : "/"); e.preventDefault(); e.target[0].value = ""}}>
                    <input className="w-[40vh] rounded-xl pl-[1vh] text-black" type="text" placeholder="search" />
                </form>
